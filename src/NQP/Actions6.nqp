...
#            }
    ...
    method package_def($/) {
        ...
        if $*PKGDECL eq 'role' {
            ...
            if $*PKGDECL eq 'grammar' {
                if can($package, '!precompute_nfas') {
            ...
            my $code := $*CODE_OBJECT;
            $*W.attach_signature($code, $*W.create_signature(hash('parameter_objects', [])));
            $*W.finish_code_object($code, $block, 0);
            $*W.add_phasers_handling_code($code, $block);
        }
    ...
    sub begin_time_lexical_fixup($block) {
        ...
        my $throwaway_block := $*W.create_code_object($throwaway_block_past,
            'Block', $*W.create_signature(hash('parameter_objects', [])));
    ...
    method scope_declarator:sym<HAS>($/)     {
        ...
        bindattr_i($attr, $attr.WHAT, '$!inlined', 1);
        make $scoped;
    }
    ...
    method declarator($/) {
        if    $<routine_declarator>  { make $<routine_declarator>.ast  }
            ...
            for @params {
                my $*OFTYPE := $common_of;
                if existskey($_, 'of_type') && existskey($_, 'of_type_match') {
                    if $common_of {
    ...
    sub check_default_value_type($/, $descriptor, $bind_constraint, $what) {
        unless istype($descriptor.default, $bind_constraint) {
            $*W.throw($/, 'X::Syntax::Variable::MissingInitializer',
                type => how($bind_constraint).name($bind_constraint),
                implicit => !istype($*OFTYPE, NQPMatch) || !$*OFTYPE<colonpairs> || $*OFTYPE<colonpairs> && !$*OFTYPE<colonpairs>.ast<D> && !$*OFTYPE<colonpairs>.ast<U>
    ...
    method variable_declarator($/) {
        ...
        if $desigilname ~~ /\w ':' <![:]>/ {
            ...
            $desigilname := substr($name, chars($sigil ~ $twigil));
        }
    ...
    sub declare_variable($/, $past, $sigil, $twigil, $desigilname, $trait_list, $shape?, :@post) {
        ...
        for $trait_list {
            ...
            if $trait {
                ...
                if $mod eq '&trait_mod:<is>' {
                    my @args := $trait.args;
                    if elems(@args) == 1 && !isconcrete(@args[0]) {
                        $have_is_type := 1;
                        $is_type := @args[0];
                        next;
                    }
        ...
        if $*SCOPE eq 'has' {
            ...
            unless can($package.HOW, 'add_attribute') {
                if $*PKGDECL {
            ...
            if $*SCOPE eq 'our' {
                ...
                if can($package.HOW, 'archetypes') && $package.HOW.archetypes.parametric {
            ...
            if $past.isa(QAST::Var) {
                ...
                $past.scope(objprimspec($bind_type) ?? 'lexicalref' !! 'lexical');
            ...
            if @late_traits {
                ...
                my $varvar   := create($Variable);
                bindattr_s($varvar, $Variable, '$!name', $name);
                bindattr_s($varvar, $Variable, '$!scope', $*SCOPE);
                bindattr($varvar, $Variable, '$!var', $cont);
                bindattr($varvar, $Variable, '$!block', $*CODE_OBJECT);
                bindattr($varvar, $Variable, '$!slash', $/);
    ...
    method routine_def($/) {
        ...
        if $*SIG_OBJ {
            ...
            $signature := $*W.create_signature_and_params($/,
                hash('parameters', @params), $block, 'Any');
        }
        add_signature_binding_code($block, $signature, @params);
        ...
        if $<deflongname> {
            ...
            if $*MULTINESS eq 'multi' {
                ...
                unless can($proto, 'is_dispatcher') && $proto.is_dispatcher {
                ...
                if $predeclared {
                    my $Routine := $*W.find_symbol(['Routine'], :setting-only);
                    unless istype(   $predeclared<value>, $Routine)
                        && getattr_i($predeclared<value>, $Routine, '$!yada') {
                        $*W.throw($/, ['X', 'Redeclaration'],
                                symbol => ~$<deflongname>.ast,
                                what   => 'routine',
                        );
    ...
    method autogenerate_proto($/, $name, $install_in) {
        ...
        my $p_sig := $*W.create_signature(hash('parameter_objects',
    ...
    method maybe_add_inlining_info($/, $code, $sig, $past, @params) {
        ...
        return 0 if can($code, 'CALL-ME');
        my $phasers := getattr($code,$*W.find_symbol(['Block'], :setting-only),'$!phasers');
        return 0 unless isnull($phasers) || !p6bool($phasers);
        ...
        my @p_objs := getattr($sig, $*W.find_symbol(['Signature'], :setting-only), '@!params');
        my int $i  := 0;
        my int $n  := elems(@params);
        my %arg_placeholders;
        while $i < $n {
            my %info := @params[$i];
            return 0 unless objprimspec(%info<nominal_type>); # non-native
            ...
            my int $flags := getattr_i($param_obj, $Param, '$!flags');
        ...
        for @($past[0]) {
            if istype($_, QAST::Var) && $_.scope eq 'lexical' {
                my $name := $_.name;
                return 0 if $name ne '$*DISPATCHER' && $name ne '$_' &&
                    $name ne '$/' && $name ne '$¢' && $name ne '$!' &&
                    !existskey(%arg_placeholders, $name);
            }
            elsif istype($_, QAST::Block) {
                return 0;
            }
            elsif istype($_, QAST::Stmt) && istype($_[0], QAST::Block) {
                return 0;
            }
        }
        ...
        sub node_walker($node) {
            ...
            # removing any :node(...).
            if istype($node, QAST::IVal) || istype($node, QAST::SVal)
            || istype($node, QAST::NVal) {
            ...
            elsif istype($node, QAST::WVal) {
                if $node.value =:= $PseudoStash {
                    die("Routines using pseudo-stashes are not inlinable");
                }
                else {
                    return $node.node ?? clear_node($node.shallow_clone()) !! $node;
                }
            ...
            # themselves, it comes down to the children.
                ...
                if getcomp('QAST').operations.is_inlinable('perl6', $node.op) {
                    ...
                    die("Non-inlinable op '" ~ $node.op ~ "' encountered");
                }
            }

            # Variables are fine *if* they are arguments.
            elsif istype($node, QAST::Var) && ($node.scope eq 'lexical' || $node.scope eq '') {
                if existskey(%arg_placeholders, $node.name) {
                    ...
                    die("Cannot inline with non-argument variables");
                }
            }

            # Statements need to be cloned and then each of the nodes below them
            # visited.
            elsif istype($node, QAST::Stmt) || istype($node, QAST::Stmts) {
                my $replacement := $node.shallow_clone();
                my int $i := 0;
                my int $n := +@($node);
                while $i < $n {
            ...
            # Otherwise, we don't know what to do with it.
                ...
                die("Unhandled node type; won't inline");
            }
        };
        my $inline_info;
        try $inline_info := node_walker($past[1]);
        return 0 unless istype($inline_info, QAST::Node);
    ...
    sub is_clearly_returnless($block) {
        sub returnless_past($past) {
            return 0 unless
                ...
                istype($past, QAST::Op)
                    ...
                    && getcomp('QAST').operations.is_inlinable('perl6', $past.op) ||
                # Just a variable lookup.
                istype($past, QAST::Var) ||
                # Just a QAST::Want
                istype($past, QAST::Want) ||
                # Just a primitive or world value.
                istype($past, QAST::WVal) ||
                istype($past, QAST::IVal) ||
                istype($past, QAST::NVal) ||
                istype($past, QAST::SVal);
            for @($past) {
                if istype($_, QAST::Node) {
        ...
        if +$block[1].list == 1 && istype($block[1][0], QAST::Stmt) && +$block[1][0].list == 1 {
            # Ensure there's no nested blocks.
            for @($block[0]) {
                if istype($_, QAST::Block) { return 0; }
                if istype($_, QAST::Stmts) {
                    for @($_) {
                        if istype($_, QAST::Block) { return 0; }
                    }
                }
        ...
        elsif +$block[1].list == 1 && istype($block[1][0], QAST::WVal) {
            1
        }
        else {
            0
    ...
    sub regex_coderef($/, $code, $qast, $scope, $name, %sig_info, $block, $traits?, :$proto, :$use_outer_match) {
        ...
        if $match {
            $source := substr($source, 0, $match.from());
        }
        bindattr_s($code, $Regex, '$!source', $source);
    ...
    method type_declarator:sym<enum>($/) {
        ...
        my $cur_value := box_i(-1, $*W.find_symbol(['Int']));
        my @redecl;
        my $block := $*W.cur_lexpad();
        for @values {
            ...
            # key.
            my $cur_key;
            if istype($_, QAST::Node) && istype($_.returns(), $Pair) {
            ...
            unless istype($cur_key, $*W.find_symbol(['Str'])) {
                $cur_key := $cur_key.Str;
            }
            unless defined($cur_value) {
                $*W.throw($/, 'X::Comp::NYI',
        ...
        if +@redecl -> $amount {
            if $amount > 2 {
                @redecl[$amount - 2] := @redecl[$amount - 2] ~ ' and ' ~ pop(@redecl);
                $/.typed_worry('X::Redeclaration', symbol => join(', ', @redecl));
            }
            elsif $amount > 1 {
                $/.typed_worry('X::Redeclaration', symbol => join(' and ', @redecl));
            }
            else {
    ...
    method parameter($/) {
        ...
        if $<type_constraint> {
            ...
            elsif %*PARAM_INFO<sigil> eq '&' && existskey(%*PARAM_INFO, 'subsig_returns')
    ...
    sub check_param_default_type($/, $value) {
        if existskey(%*PARAM_INFO, 'nominal_type') {
            my $expected := %*PARAM_INFO<nominal_type>;
            if objprimspec($expected) == 0 {
                unless istype($value, $expected) {
    ...
    method param_var($/) {
        if $<signature> {
            if existskey(%*PARAM_INFO, 'sub_signature_params') {
            ...
            if $need_role {
                if existskey(%*PARAM_INFO, 'nominal_type') {
                    %*PARAM_INFO<nominal_type> := $*W.parameterize_type_with_args($/,
                        $role_type, [%*PARAM_INFO<nominal_type>], hash());
                }
                else {
            ...
            if $twigil eq '' || $twigil eq '*' {
                ...
                if $<name> {
                    my $name := ~$/;
                    if $<name><sigterm> {
                        $name := substr($name, 0, chars($name) - chars(~$<name><sigterm>));
        ...
        if istype($*PRECEDING_DECL, $par_type) {
    ...
    method named_param($/) {
        %*PARAM_INFO<named_names> := %*PARAM_INFO<named_names> || list_s();
        if $<name>               { push_s(%*PARAM_INFO<named_names>, ~$<name>); }
        elsif $<param_var><name> { push_s(%*PARAM_INFO<named_names>, ~$<param_var><name>); }
        else                     { push_s(%*PARAM_INFO<named_names>, ''); }
    }
    ...
    method type_constraint($/) {
        if $<typename> {
            ...
            if eqat($typename, '::', 0) && !eqat($typename, '?', 2) {
                # Set up signature so it will find the typename.
                my str $desigilname := substr($typename, 2);
                unless %*PARAM_INFO<type_captures> {
                    %*PARAM_INFO<type_captures> := list_s()
                }
                push_s(%*PARAM_INFO<type_captures>, $desigilname);
    ...
    sub dissect_type_into_parameter($/, $type) {
        if isconcrete($type) {
            if istype($type, $*W.find_symbol(['Bool'])) {
                my $val := $type.gist;
                $/.worry(
    ...
    my class Trait {
        ...
        method new($match, str $trait_mod, *@pos_args, *%named_args) {
            my $self := create(self);
            bindattr($self, Trait, '$!match', $match);
            bindattr_s($self, Trait, '$!trait_mod', $trait_mod);
            bindattr($self, Trait, '@!pos_args', @pos_args);
            bindattr($self, Trait, '%!named_args', %named_args);
            $self
        }
    ...
    method trait_mod:sym<is>($/) {
        ...
        if ~$<longname> eq 'repr' {
            if $<circumfix> {
                if istype($<circumfix>[0].ast, QAST::WVal) {
    ...
    method privop($/) {
        ...
        if $<methodop><longname> {
            ...
            if @parts {
                ...
                unless can($methpkg.HOW, 'is_trusted') && $methpkg.HOW.is_trusted($methpkg, $package) {
                ...
                unless can($package.HOW, 'find_private_method') {
    ...
    sub expand_macro($macro, $name, $/, &collect_argument_asts) {
        ...
        my $macro_ast_qast := getattr(
            decont($macro_ast),
            $ast_class,
            '$!past'
        );
        unless defined($macro_ast_qast) {
            return QAST::WVal.new( :value($*W.find_symbol(['Nil'])) );
        }
        my $block := {{ :raw $macro_ast_qast }};
        $*W.add_quasi_fixups($macro_ast, $block);
        AST {{ $block, call BVal $block }};
    }
    ...
    sub add_macro_arguments($expr, @argument_asts, $code_string) {
        ...
        sub wrap_and_add_expr($expr) {
            my AST $quasi_ast := $ast_class.new();
            my $wrapped := AST call( make_thunk_ref($expr, $expr.node) );
            $quasi_ast!$!past := $wrapped;
            $quasi_ast!$!Str :=  $code_string);
    ...
    method term:sym<name>($/) {
        ...
        if $*longname.contains_indirect_lookup() {
            ...
            # and make the call.
            # Add & to name.
            my @name := clone($*longname.attach_adverbs.components);
            ...
            if $macro {
                ...
                if +@name == 1 {
                    ...
                    if +$past.list == 1 && %commatrap{@name[0]} {
                        ...
                        my $pre := substr($/.orig, $<longname>.from - $prelen, $prelen);
                        my $post := substr($/.orig, $<args>.to, 100);
                        if index($pre, "==>") < 0 && index($post, "<==") < 0 && $<args>.Str ~~ /^\s*['{'|'->'|'<->']/ {
                            $/.missing("comma after block argument to " ~ substr(@name[0],1));
                        }
                    }
    ...
    method term:sym<op>($/) {
        ...
        $past.node($/);
        getcomp('QAST').operations.attach_result_type('perl6', $past);
        make $past;
    }
    ...
    sub hunt_loose_adverbs_in_arglist($/, @past) {
        ...
        if getenvhash<COLONPAIR> eq 'trace' { say($/.dump) }
        if +$/.list == 1 && istype($/[0].ast, QAST::Op) && $/[0].ast.op eq 'call' && $/[0].ast.name ne 'infix:<,>' {
            die("these adverbs belong to a deeper-nested thing");
        }
        if $<fake_infix>
        ...
        if $<EXPR> {
            ...
            if istype($expr, QAST::Op) {
                if $expr.name eq '&infix:<,>' {
                    ...
                    if $expr2 {
                        if istype($expr2, QAST::Op) && $expr2.name eq '&infix:<,>' {
                            @args := $expr2.list;
                        }
                        else {
                            @args := [$expr2];
            ...
            my %named_counts;
            for @args {
                if istype($_, QAST::Op) && istype($_.returns, $Pair)
                    && can($_[1], 'has_compile_time_value') {
    ...
    method circumfix:sym<( )>($/) {
        ...
        if $size == 0 {
            ...
            if $size == 1
            && istype($past[0],    QAST::Op)  && $past[0].op eq 'callmethod' && $past[0].name eq 'new'
            && istype($past[0][0], QAST::Var) && $past[0][0].name eq 'Pair' {
    ...
    method circumfix:sym<{ }>($/) {
        ...
        if $bast.symbol('$_')<used> || $bast.ann('also_uses') && $bast.ann('also_uses')<$_> {
            ...
            $elem := $elem[0] if istype($elem, QAST::Op) && $elem.op eq 'p6fatalize';
            ...
            while $subelem ~~ QAST::Op && ($subelem.op eq 'p6capturelex' || $subelem.op eq 'if' || $subelem.op eq 'unless') {
                ...
                if $subelem ~~ QAST::Op && $subelem.op eq 'callmethod' && $subelem.name eq 'clone' {
                    $subelem := $subelem[0];
                    if $subelem ~~ QAST::WVal && istype($subelem.value, $*W.find_symbol(['WhateverCode'], :setting-only)) {
        ...
        if $is_hash && $past.ann('past_block').arity == 0 {
            ...
            $past := QAST::Op.new(
                :op('call'),
                :name(
                    $/.from && eqat($/.orig, ':', $/.from - 1) ?? '&circumfix:<:{ }>' !! '&circumfix:<{ }>'
                ),
                :node($/)
            );
            if $has_stuff {
                for @children {
                    if istype($_, QAST::Stmt) {
    ...
    sub migrate_blocks($from, $to, $predicate?) {
        my @decls := @($from[0]);
        my int $n := elems(@decls);
        my int $i := 0;
        while $i < $n {
            my $decl := @decls[$i];
            if istype($decl, QAST::Block) {
                if !$predicate || $predicate($decl) {
                    $to[0].push($decl);
                    @decls[$i] := QAST::Op.new( :op('null') );
                }
            }
            elsif (istype($decl, QAST::Stmt) || istype($decl, QAST::Stmts)) &&
                  istype($decl[0], QAST::Block) {
                if !$predicate || $predicate($decl[0]) {
                    $to[0].push($decl[0]);
                    $decl[0] := QAST::Op.new( :op('null') );
                }
            }
            elsif istype($decl, QAST::Var) && $predicate && $predicate($decl) {
                $to[0].push($decl);
                @decls[$i] := QAST::Op.new( :op('null') );
            }
            $i++;
    ...
    method EXPR($/, $KEY?) {
        ...
        elsif $past && eqat($past.name, '&METAOP_TEST_ASSIGN', 0) {
            my $test_type;
        ...
        if $key eq 'postfix' {
            ...
            if $<colonpair> {
                ...
                if istype($target, QAST::Op) && $target.op eq 'hllize' {
                    $target := $target[0];
                }
                unless istype($target, QAST::Op) && ($target.op eq 'call' || $target.op eq 'callmethod') {
                    if can($target, 'name') {
                ...
                if istype($past, QAST::Op) && $past.op eq 'hllize' {
            ...
            $key := 'infix'
                if istype($past, QAST::Op) && $past.op eq 'call' && $past.name eq '&postfix:<ⁿ>';
            ...
            for $/.list { if $_.ast { WANTED($_.ast, "EXPR/thunky") if eqat($thunky,'.',$arity); $past.push($_.ast); ++$arity; } }
        }
        else {
...
#        if atkey(getenvhash,'RAKUDO_EXPR') {
#            note("$key $sym");
...
#        }
    ...
    sub make_feed($/) {
        ...
        for @stages {
            ...
            if $stage.isa(QAST::Op) && $stage.op eq 'call' {
                ...
                if $stage.isa(QAST::Op) && $stage.op eq 'ifnull' && $stage[0].isa(QAST::Var) && eqat($stage[0].name, '&', 0) {
                    $error := "A feed may not sink values into a code object. Did you mean a call like '"
                            ~ substr($stage[0].name, 1) ~ "()' instead?";
                }
    ...
    sub check_smartmatch($/,$pat) {
        if can($pat,'ann') && $pat.ann('is_S') {
            $/.PRECURSOR.worry('Smartmatch with S/// is not useful. You can use given instead: S/// given $foo');
        }

        if $pat ~~ QAST::WVal && istype($pat.returns, $*W.find_symbol(['Bool'])) && isconcrete($pat.value) {
    ...
    sub make_smartmatch($/, $negated) {
        ...
        return 0 if $lhs ~~ QAST::WVal && istype($lhs.returns, $*W.find_symbol(['Whatever'])) && isconcrete($lhs.value);
        return 0 if $rhs ~~ QAST::WVal && istype($rhs.returns, $*W.find_symbol(['Whatever'])) && isconcrete($rhs.value);

        # don't need topicalization, so allow chaining?
        return 0 if !$*COMPILING_CORE_SETTING && (
            $rhs.has_compile_time_value ||
            istype($rhs,QAST::Var) ||
            istype($lhs,QAST::Op) && $lhs.op eq 'chain'
        );
    ...
    sub bind_op($/, $target, $source, $sigish) {
        ...
        if $target.isa(QAST::Var) {
            ...
            my $sigil := substr($target.name(), 0, 1);
            if $sigil eq '@' || $sigil eq '%' {
        ...
        elsif $target.isa(QAST::WVal) && istype($target.value, $*W.find_symbol(['Signature'], :setting-only)) {
            make QAST::Op.new(
                :op('p6bindcaptosig'),
                $target,
                QAST::Op.new(
    ...
    sub assign_op($/, $lhs_ast, $rhs_ast) {
        ...
        if istype($lhs_ast, QAST::Var) {
            $var_sigil := substr($lhs_ast.name, 0, 1);
            if $var_sigil eq '%' {
                if can($rhs_ast,'name') {
        ...
        if istype($lhs_ast, QAST::Var)
                && objprimspec($lhs_ast.returns) -> $spec {
    ...
    sub mixin_op($/, $sym) {
        ...
        if $rhs.isa(QAST::Op) && $rhs.op eq 'call' {
            if $rhs.name && +@($rhs) == 1 {
                try {
                    $past.push(QAST::WVal.new( :value($*W.find_symbol([substr($rhs.name, 1)])) ));
                    $rhs[0].named('value');
                    $past.push($rhs[0]);
                    CATCH { $past.push($rhs); }
                }
    ...
    sub thunkity_thunk($/,$thunky,$past,@clause) {
        my int $i := 0;
        my int $e := elems(@clause);
        my int $te := chars($thunky);
        my $type := substr($thunky,0,1);
        while $i < $e {
            my $ast := @clause[$i];
            $ast := $ast.ast if can($ast,'ast');  # reduce already passes ast...

            if $type eq 'T' || $type eq 'B' || $type eq 'A' {
                my $argast := $ast;
                $argast := $argast[0] if istype($argast,QAST::Stmts);
                if istype($argast,QAST::Op) && $argast.op eq 'call' && $argast.name eq '&infix:<,>' {
#                    note("thunky $type bingo:\n" ~ $argast.dump);
                    my int $ae := elems($argast);
                    my int $a := 0;
                    while $a < $ae {
            ...
            if $type eq '.' || $type eq 'T' || $type eq 'B' || $type eq 'A' || istype($ast,QAST::SpecialArg) {
            ...
            $type := substr($thunky,$i,1) if ++$i < $te;  # repeat last thunk spec as necessary
        }
        $past;
    }
        ...
        if $<infix> {
            ...
            my $basepast := $base.ast
                              ...
                              !! QAST::Var.new(:name("&infix" ~ $*W.canonicalize_pair('', $basesym)),
                                               :scope<lexical>);
            my $purity := 0;
            if istype($basepast, QAST::Var) {
                my $subfun := try $*W.find_symbol([$basepast.name]);
                if $subfun {
                    $purity := 1 if can($subfun, 'IS_PURE') && $subfun.IS_PURE;
                }
                else {
            ...
            if    $metasym eq 'R' { $helper := '&METAOP_REVERSE'; $t := flip($t) if $t; }
            elsif $metasym eq 'X' { $helper := '&METAOP_CROSS'; $t := uc($t); }  # disable transitive thunking for now
            elsif $metasym eq 'Z' { $helper := '&METAOP_ZIP'; $t := uc($t); }
    ...
    method postfix:sym<ⁿ>($/) {
        ...
        my $power := box_i(0, $Int);
        for $<dig> {
            $power := add_I(
                mul_I($power, box_i(10, $Int), $Int),
                box_i(index("⁰¹²³⁴⁵⁶⁷⁸⁹", $_), $Int),
                $Int);
        }

        $power := neg_I($power, $Int) if $<sign> eq '⁻' || $<sign> eq '¯';
    ...
    method postcircumfix:sym<[ ]>($/) {
        ...
        if $<semilist> {
            my $c := $/;
            my $ast := $<semilist>.ast;
            $past.push($ast) if istype($ast, QAST::Stmts);
    ...
    method postcircumfix:sym<ang>($/) {
        ...
        my $nib  := $<nibble>.ast;
        $past.push($nib)
            unless istype($nib, QAST::Stmts) &&
                   istype($nib[0], QAST::Op) && $nib[0].name eq '&infix:<,>' && +@($nib[0]) == 0;
        make $past;
    }
    ...
    method numish($/) {
        if $<integer> {
            ...
            my $code := ord($/.Str);
            my int $nu := +getuniprop_str($code, unipropcode("Numeric_Value_Numerator"));
            my int $de := +getuniprop_str($code, unipropcode("Numeric_Value_Denominator"));
            if !$de || $de == 1 {
            ...
            make $*W.add_numeric_constant($/, 'Num', inf);
        }
        else {
    ...
    method dec_number($/) {
        ...
        if chars($<frac>) {
            $partf := radix_I(10, $<frac>.Str, 0, 4, $Int);

            $parti := mul_I($parti, $partf[1], $Int);
            $parti := add_I($parti, $partf[0], $Int);

            $partf := tonum_I($partf[1]);
        } else {
            $partf := 1.0;
        }
        ...
        if $<escale> { # wants a Num
            ...
            $parti := div_n($parti, $partf);
            if $parti != 0.0 {
                $parti := mul_n($parti, pow_n(10, tonum_I($<escale>.ast)));
            }

            make $*W.add_numeric_constant($/, 'Num', $parti);
        } else { # wants a Rat
            my $ast := $*W.add_constant('Rat', 'type_new', $parti, fromnum_I($partf, $Int), :nocache(1));
            $ast.node($/);
            make $ast;
        }
    }
    ...
    method rad_number($/) {
        ...
        if $<bracket> { # the "list of place values" case
            ...
            my $ipart := radix_I($radix, $<intpart>.Str, 0, 0, $Int);
            my $fpart := radix_I($radix, chars($<fracpart>) ?? $<fracpart>.Str !! ".0", 1, 4, $Int);
            my $bpart := $<base> ?? tonum_I($<base>[0].ast) !! $radix;
            my $epart := $<exp> ?? tonum_I($<exp>[0].ast) !! 0;

            if $ipart[2] < chars($<intpart>.Str) {
            ...
            if $fpart[2] < chars($<fracpart>.Str) {
                $*W.throw($/, 'X::Str::Numeric',
                    :source($<intpart> ~ ($<fracpart> // '')),
                    :reason("malformed base-$radix number"),
                    :pos( # the -1 dance is due to radix returning -1 for
            ...
            $ipart := mul_I($ipart[0], $fpart[1], $Int);
            $ipart := add_I($ipart, $fpart[0], $Int);
            $fpart := $fpart[1];

            my $scientific := pow_n($bpart, $epart);
            $ipart := mul_I($ipart, fromnum_I($scientific, $Int), $Int);
    ...
    method typename($/) {
        ...
        if $<longname> {
            ...
            if !eqat($str_longname, '::', 0) {
                ...
                if $*W.cur_lexpad.symbol(substr($str_longname, 2)) {
                    $*W.throw($/, ['X', 'Redeclaration'],
                        symbol => substr($str_longname, 2));
                }
                make $*W.pkg_create_mo($/, $*W.resolve_mo($/, 'generic'), :name(substr($str_longname, 2)));
            }
        }
        else {
    ...
    method rx_adverbs($/) {
        my @pairs;
        for $<quotepair> {
            push(@pairs, wanted($_.ast,'rx_adverbs'));
        }
        make @pairs;
    }
    ...
    method quote:sym<quasi>($/) {
        ...
        bindattr($quasi_ast, $ast_class, '$!past', $past);
        bindattr($quasi_ast, $ast_class, '$!Str', $/.Str());
    ...
    sub add_signature_binding_code($block, $sig_obj, @params) {
        ...
        unless defined($use_vm_binder) {
            $use_vm_binder := getcomp('perl6').backend.name eq 'moar';
        }
        if $use_vm_binder {
    ...
    sub is_anon_capture(@params) {
        if elems(@params) == 1 {
            my $only := @params[0];
            if $only<is_capture> && !existskey($only, 'variable_name')
                                 && !existskey($only, 'sub_signature')
                                 && !existskey($only, 'post_constraints') {
                if !istype($*W.find_symbol(['Capture']), $only<nominal_type>) {
    ...
    sub lower_signature($block, $sig, @params) {
        ...
        my @p_objs := getattr($sig, $Sig, '@!params');
        my int $i  := 0;
        my int $n  := elems(@params);
        while $i < $n {
            ...
            my $param_obj := @p_objs[$i];
            my int $flags := getattr_i($param_obj, $Param, '$!flags');
            return 0 if existskey(%info, 'sub_signature');
            ...
            if %info<is_capture> {
                ...
                if !istype($*W.find_symbol(['Capture']), %info<nominal_type>) {
                ...
                if existskey(%info, 'variable_name') {
            ...
            elsif existskey(%info, 'named_names') {
                my @names := %info<named_names>;
                if elems(@names) == 1 {
                    $var.named(atpos_s(@names, 0));
                }
                elsif elems(@names) == 2 {
                    my @names_copy;
                    @names_copy[0] := atpos_s(@names, 0);
                    @names_copy[1] := atpos_s(@names, 1);
                ...
                if existskey(%info, 'variable_name') && %info<variable_name> eq '%_' {
            ...
            my int $spec  := objprimspec($nomtype);
            ...
            # Handle coercion.
            my $coerce_to := getattr($param_obj, $Param, '$!coerce_type');
            unless isnull($coerce_to) {
                ...
                $var.push(QAST::Op.new(
                    ...
                    QAST::Op.new(
                        ...
                        QAST::Op.new(
                            :op('callmethod'),
                            :name(getattr_s($param_obj, $Param, '$!coerce_method')),
                            QAST::Var.new( :name($name), :scope('local') )
                        ))));
            }
            ...
            if $flags +& $SIG_ELEM_IS_OPTIONAL {
                if existskey(%info, 'default_value') {
                    ...
                    if (my $is-array := %info<sigil> eq '@') || %info<sigil> eq '%' {
                        ...
                        $var.default(
                            QAST::Op.new(
                                :op<create>,
                                QAST::WVal.new(
                                    value => istype($nomtype, $*W.find_symbol([$role])) && can($nomtype.HOW, 'role_arguments')
                                               ?? $*W.parameterize_type_with_args($/, $*W.find_symbol([$base-type]), $nomtype.HOW.role_arguments($nomtype), hash)
            ...
            if %info<type_captures> {
                my $iter := iterator(%info<type_captures>);
                while $iter {
                    $var.push( QAST::Op.new(
                        :op<bind>,
                        QAST::Var.new( :name(shift($iter)), :scope<lexical> ),
                        QAST::Op.new( :op<what>,
                            QAST::Var.new( :name($name), :scope<local> ) )
                        )
                    );
            ...
            if existskey(%info, 'variable_name') && !%info<bind_attr> {
                if objprimspec($nomtype) || $is_rw || $flags +& $SIG_ELEM_IS_RAW {
                    ...
                    unless $wrap {
                        $wrap := istype($nomtype, $Iterable) || istype($Iterable, $nomtype);
                    }
                    if $wrap {
                        $var.push(QAST::Op.new(
                            :op('bind'),
                    ...
                    if %info<variable_name> eq '$_' && $*IMPLICIT {
                        for $block[0].list {
                            if istype($_, QAST::Op) && $_.op eq 'bind' &&
                               istype($_[0], QAST::Var) && $_[0].name eq '$_' {
                                $clear_topic_bind := $_;
                                last;
                            }
                        }
            ...
            if %info<post_constraints> {
                for %info<post_constraints> {
                    ...
                    $var.push(QAST::ParamTypeCheck.new(QAST::Op.new(
                        ...
                        QAST::Op.new(
                            :op('callmethod'), :name('ACCEPTS'),
                            istype($_, $*W.find_symbol(['Code'], :setting-only))
    ...
    sub find_var_decl($block, $name) {
        for $block[0].list {
            if istype($_, QAST::Var) && $_.name eq $name && $_.decl {
                return $_;
            }
        }
        die("Internal error: find_var_decl could not find $name");
    }
    ...
    sub add_placeholder_parameter($/, $sigil, $ident, :$named, :$pos_slurpy, :$named_slurpy, :$full_name) {
        ...
        if chars($full_name) == 3 && substr($full_name,2,1) ~~ /^<[A..Z]>$/ {
        ...
        # If it's named, just shove it on the end, but before any slurpies.
            ...
            %param_info<named_names> := list_s($ident);
            my @popped;
            while @params
        ...
        # Otherwise, put it in correct lexicographic position.
            ...
            for @params {
                last if $_<pos_slurpy> || $_<named_slurpy> ||
                        $_<named_names> ||
                        substr($_<variable_name>, 1) gt $ident;
                $insert_at := $insert_at + 1;
            }
            splice(@params, [%param_info], $insert_at, 0);
        }
    ...
    our sub make_thunk_ref($to_thunk, $/) {
        ...
        if !$*SUPPOSING {  # don't actually copy the thunk if inside <?before>
            fatalize($to_thunk) if $*FATAL || (can($/,'CURSOR') ?? $/.pragma('fatal') !! $*LANG.pragma('fatal'));
    ...
    sub make_topic_block_ref($/, $past, :$copy, :$migrate_stmt_id) {
        ...
        my $sig := $*W.create_signature(hash('parameter_objects', [$param_obj]));
    ...
    sub make_where_block($/, $expr, $operand = WANTED(QAST::Var.new( :name('$_'), :scope('lexical') ),'where') ) {
        ...
        my $param := hash(
            variable_name => '$_',
            nominal_type => $*W.find_symbol(['Mu']));
        my $sig := $*W.create_signature(hash('parameter_objects',
    ...
    sub when_handler_helper($when_block) {
        unless existskey(%*HANDLERS, 'SUCCEED') {
    ...
    sub set_block_handler($/, $handler, $type) {
        ...
        # If the handler has a succeed handler, then make sure we sink
        # the exception it will produce.
        if $handler.ann('past_block').ann('handlers') && existskey($handler.ann('past_block').ann('handlers'), 'SUCCEED') {
        ...
        if $handler.ann('past_block').ann('handlers') && existskey($handler.ann('past_block').ann('handlers'), $type) {
        }
        else {
    ...
    sub capture_or_raw($/,$args, $name) {
        ...
        if $name eq 'return' {
            my $ret := %*SIG_INFO<returns>;
            if isconcrete($ret) || $ret.HOW.name($ret) eq 'Nil' {
                if elems($args) {
            ...
            my $all_literal := 1;
            for @($args) {
                $all_literal := 0 unless istype($_,QAST::SpecialArg) ||
                    istype($_,QAST::Want) && istype($_[0],QAST::WVal) && $_[1] eq 'Ss' && istype($_[2],QAST::SVal);
            }
    ...
    method whatever_curry($/, $past, $upto_arity) {
        my int $curried :=
            ...
            # Either a call that we're allowed to curry...
                (($past.op eq 'call' || $past.op eq 'chain') &&
                    (eqat($past.name, '&infix:', 0) ||
                     eqat($past.name, '&prefix:', 0) ||
                     eqat($past.name, '&postfix:', 0) ||
                     (istype($past[0], QAST::Op) &&
                        eqat($past[0].name, '&METAOP', 0))) &&
                    %curried{$past.name} // 3)
            ...
            # or one of our new postcircumfix subs that used to be methods
                || ($past.op eq 'call' && eqat($past.name, '&postcircumfix:', 0) &&
                    %curried{$past.name} // 0)
            );
        ...
        my int $offset := 0;
        $offset := elems($past) - $upto_arity if $past.op eq 'call' && !eqat($past.name,"&postcircumfix:",0);
        my int $i := $offset;
        ...
        while $i < $e {
            my $check := $past[$i];
            $check := $check[0] if (istype($check, QAST::Stmts) ||
                                    istype($check, QAST::Stmt)) &&
                                   +@($check) == 1;
            $whatevers++ if bitand_i($curried, 1) && istype($check.returns, $Whatever) && isconcrete($check.value)
                         || bitand_i($curried, 2) && istype($check.returns, $WhateverCode) && $check ~~ QAST::Op;
            if bitand_i($curried, 1) && istype($check.returns, $HyperWhatever) {
                $hyperwhatever := 1;
                $whatevers++;
            }
            $i++;
        ...
        if $whatevers {
            ...
            while $i < $e {
                my $old := $past[$i];
                $old := $old[0] if (istype($old, QAST::Stmts) ||
                                    istype($old, QAST::Stmt)) &&
                                   +@($old) == 1;
                if bitand_i($curried, 2) && istype($old.returns, $WhateverCode) && $old ~~ QAST::Op {
                    my $new;
                    if $was_chain && $old.has_ann("chain_args") {
                        ...
                        while $acount < $old.arity {
                            ...
                            $new.push($to_push);
                            push(@old_args, $to_push) if $was_chain;
                            $acount++;
                        }
                    }
                    $past[$i] := $new;
                }
                elsif bitand_i($curried, 1) && (istype($old.returns, $Whatever) || istype($old.returns, $HyperWhatever)) && isconcrete($old.value) {
                    my $pname := $*W.cur_lexpad()[0].unique('$whatevercode_arg');
                    @params.push(hash(
                        :variable_name($pname),
                        :nominal_type($*W.find_symbol(['Mu'])),
                    ...
                    push(@old_args, $past[$i]) if $was_chain;
                } else {
                    push(@old_args, $past[$i]) if $was_chain;
                }
                $i++;
    ...
    sub remove_block($from, $block) {
        ...
        while $i < $n {
            ...
            elsif istype($consider, QAST::Stmt) || istype($consider, QAST::Stmts) {
                if $consider[0] =:= $block {
        ...
        die('Internal error: failed to remove block');
    }

    sub wrap_return_type_check($wrappee, $code_obj) {
        my $ret := %*SIG_INFO<returns>;
        return $wrappee if isconcrete($ret) || $ret.HOW.name($ret) eq 'Nil';
        QAST::Op.new(
            :op('p6typecheckrv'),
            $wrappee,
    ...
    sub instantiated_type(@name, $/) {
        ...
        my $past;
        if $is_generic || isnull(getobjsc($type)) || istype($type.HOW,$/.how('package')) {
            $past := $*W.symbol_lookup(@name, $/);
    ...
    sub compile_time_value_str($past, $usage, $/) {
        if $past.has_compile_time_value {
            unbox_s($past.compile_time_value);
        }
        else {
...
class Perl6::QActions is HLL::Actions does STDActions {
    ...
    method charname-notfound($/) {
        ...
        if ~$/ eq "LINE FEED (LF)" {
            $/.worry(sprintf($text, (~$/, @worry-text[0]) ) );
            return getstrfromname("LINE FEED");
        }
        if ~$/ eq "FORM FEED (FF)" {
            $/.worry(sprintf($text, (~$/, @worry-text[1]) ) );
            return getstrfromname("FORM FEED");
        }
        if ~$/ eq "CARRIAGE RETURN (CR)" {
            $/.worry(sprintf($text, (~$/, @worry-text[2]) ) );
            return getstrfromname("CARRIAGE RETURN");
        }
        if ~$/ eq "NEXT LINE (NEL)" {
            $/.worry(sprintf($text, (~$/, @worry-text[3]) ) );
            return getstrfromname("NEXT LINE");
        }
    ...
    method nibbler($/) {
        ...
        for @*nibbles {
            if istype($_, NQPMatch) {
                if istype($_.ast, QAST::Node) {
                    if $lastlit ne '' {
    ...
    method postprocess_val($/, $qast) {
        if istype($qast, QAST::Stmts) && istype($qast[0], QAST::Op) && $qast[0].name eq '&infix:<,>' { # qw/qqww list
            my @results := [];
            ...
            for $qast[0].list -> $thisq {
                if $thisq.has_compile_time_value {
                    try {
                        ...
                        $*W.add_object($result);
                        push(@results, QAST::WVal.new(:value($result), :node($/)));

                        CATCH { push(@results, $thisq) }
                    }
                } else {
                   push(@results, QAST::Op.new(:name('&val'), :op('call'), :node($/), $thisq));
                }
            }
    ...
    method postprocess_quotewords($/, $past) {
        ...
        sub walk($node) {
            ...
            elsif istype($node, QAST::Op) && $node.name eq '&infix:<~>' {
                walk($node[0]);
                walk($node[1]);
            }
            # (can't just use postprocess_words here because it introduces spurious comma operations)
                ...
                my @words := HLL::Grammar::split_words($/,
                    unbox_s($node.compile_time_value));
    ...
    method backslash:sym<a>($/) { make chr(7) }
    ...
    method backslash:sym<n>($/) {
        my str $nl := unbox_s($*W.find_symbol(['$?NL']));
        if can($/, 'parsing_heredoc') {
...
class Perl6::RegexActions is QRegex::P6Regex::Actions does STDActions {
    ...
    method rxquote($/) {
        ...
        if $quote.has_compile_time_value {
            ...
            my $qast := QAST::Regex.new( :rxtype<literal>, unbox_s($quote.compile_time_value) );
            if %*RX<i> && %*RX<m> { # >
            ...
            ast Regex NodeList ':LITERAL', $quote, %*RX<i> ?? AST 1 !! AST 0 
    ...
    method metachar:sym<rakvar>($/) {
        ...
        if istype($varast, QAST::Var) {
            ...
            if istype($varast.returns, $*W.find_symbol(['Str'])) {
                make QAST::Regex.new(QAST::NodeList.new(
                        QAST::SVal.new( :value('!LITERAL') ),
                        $varast,
                        QAST::IVal.new( :value(%*RX<i> ?? 1 !! 0) )
    ...
    method assertion:sym<name>($/) {
        ...
        if $lng.contains_indirect_lookup() {
            ...
            if $<assertion> {
                ...
                my int $loc := index($fullrxname, ':sym<');
                if $loc < 0 {
                    $c.panic('Can only use <sym> token in a proto regex')
                        if ($loc := index($fullrxname, ':sym«')) < 0;
                }
                my str $rxname := substr($fullrxname, $loc + 5, chars($fullrxname) - $loc - 6);
                ...
                if +@parts {
                    ...
                    $qast := QAST::Regex.new(:rxtype<subrule>, :subtype<capture>,
                                             ...
                                             :name(~$<longname>) );
                } elsif $*W.regex_in_scope('&' ~ $name) && substr($c.orig, $/.from - 1, 1) ne '.' {
    ...
    method create_regex_code_object($block) {
        $*W.create_code_object($block, 'Regex',
            $*W.create_signature(hash('parameter_objects', [])))
    }
