...
# Operand data types.
my $MVM_operand_int64       := bitshiftl_i($MVM_reg_int64, 3);
my $MVM_operand_num64       := bitshiftl_i($MVM_reg_num64, 3);
my $MVM_operand_str         := bitshiftl_i($MVM_reg_str, 3);
my $MVM_operand_obj         := bitshiftl_i($MVM_reg_obj, 3);
my $MVM_operand_uint64      := bitshiftl_i($MVM_reg_uint64, 3);
...
sub register_op_desugar($name, $desugar, :$inlinable = 1) is export {
    getcomp('QAST').operations.add_hll_op('perl6', $name, :$inlinable, -> $qastcomp, $op {
...
grammar Mast {
   ...
   my %ops;
   my $ops;
  rule TOP { [ <op> | <lang> ]+  } act {
     $ops := getcomp('QAST').operations;
  }
  rule ops { ops [$<op>=\w]+ % \s } act { 
  rule op { op $<name>=\w+ <stmt>++  <end-op>
  } act {  -> $qastcomp, $op { AST

  rule stmt  { <label> | <statement > }
  rule label  {
  rule begin-op {
  }
  rule end-op { <?> } act {
     for keys %regs { $*REG_ALLOC."release_register_%regs{$_}" }
     %regs := hash;
    MAST::InstructionList.new(@ops, $cont_res.result_reg, $MVM_reg_obj)

  }
  rule reg-decl { $<name>=['$' \w+] \s* ':' $<type>=<[insu]>    } act { $*REGALLOC."fresh_$<type>"; %regs{$<name>}++ }

method release_register(


# Perl 6 opcode specific mappings.
# my $ops := getcomp('QAST').operations;
...
$ops.add_hll_op('perl6', 'p6store', -> $qastcomp, $op
    ...
    reg $iscont:i $decont:o;

    my $no_cont_lbl := MAST::Label.new();
    my $done_lbl    := MAST::Label.new();
    iscont  $iscont, $cont_res.result ));
    push(@ops, MAST::Op.new( :op('unless_i'), $iscont, ->no_cont_lbl ));
    $*REGALLOC.release_register($iscont_reg, $MVM_reg_int64);
    push(@ops, MAST::Op.new( :op('decont'), $decont_reg, $value_res.result_reg ));
    push(@ops, MAST::Op.new( :op('assign'), $cont_res.result_reg, $decont_reg ));
    $*REGALLOC.release_register($decont_reg, $MVM_reg_obj);
    push(@ops, MAST::Op.new( :op('goto'), $done_lbl ));
    
    my $meth_reg := $*REGALLOC.fresh_o();
    push(@ops, $no_cont_lbl);
    push(@ops, MAST::Op.new( :op('findmeth'), $meth_reg, $cont_res.result_reg,
        MAST::SVal.new( :value('STORE') ) ));
    push(@ops, MAST::Call.new(
        :target($meth_reg),
 ...
 ops p6var p6reprname:decont(0);


op p6definite
    my $value_res := $qastcomp.as_mast($op[0], :want($MVM_reg_obj));
    push_ilist(@ops, $value_res);
    reg $tmp:i $res:o $tcr:i $dc:o

    decont $res, $value_res.result ));
    isconcrete'), $tmp, $res ));
    push(@ops, MAST::ExtOp.new( :op('p6bool'), :cu($qastcomp.mast_compunit),
        $res, $tmp ));
    $*REGALLOC.release_register($value_res.result, $MVM_obj);
    $*REGALLOC.releaseister($tmp, $MVM_int64);
...
op p6bindassert
    ...
    push(@ops, MAST::Op.new( :op('decont'), $dc_reg, $value_res.result_reg ));
    push(@ops, MAST::Op.new( :op('istype'), $tcr_reg, $dc_reg, $type_res.result_reg ));
    push(@ops, MAST::Op.new( :op('if_i'), $tcr_reg, $lbl_done ));

    
    # Error generation.
    proto bind_error($got, $wanted) {
        my %ex := gethllsym('perl6', 'P6EX');
        if isnull(%ex) || !existskey(%ex, 'X::TypeCheck::Binding') {
            die("Type check failed in binding; expected '" ~
                $wanted.HOW.name($wanted) ~ "' but got '" ~
                $got.HOW.name($got) ~ "'");
        }
        else {
            atkey(%ex, 'X::TypeCheck::Binding')($got, $wanted)
        }
    }
    my $err_rep := $qastcomp.as_mast(QAST::WVal.new( :value(getcodeobj(&bind_error)) ));
...
ops p6stateinit p6setpre p6clearpre p6setfirstflag p6takefirstflag;


op p6return :!inlinable
    my @ops;
    my $value_res := $qastcomp.as_mast($op[0], :want($MVM_reg_obj));
    push_ilist(@ops, $value_res);
    my $ex_reg := $*REGALLOC.fresh_o();
    push(@ops, MAST::Op.new( :op('exception'), $ex_reg ));
    push(@ops, MAST::Op.new( :op('exreturnafterunwind'), $ex_reg ));
    $*REGALLOC.release_register($ex_reg, $MVM_reg_obj);
    push(@ops, MAST::Op.new( :op('return_o'), $value_res.result_reg ));
    MAST::InstructionList.new(@ops, $value_res.result_reg, $MVM_reg_obj)
});a

ops p6getouterctx p6captureouters p6captureouters2:nqp:p6captureouters
# lang nqp
# lang perl6
...
$ops.add_hll_op('perl6', 'p6argvmarray', -> $qastcomp, $op {
    ...
    push(@ops, MAST::Op.new( :op('param_sp'), $res_reg, 
        MAST::IVal.new( :value(0), :size(16) )));
    reg $i_reg    := $*REGALLOC.fresh_i();
    my $n_reg    := $*REGALLOC.fresh_i();
    my $cmp_reg  := $*REGALLOC.fresh_i();
    my $tmp_reg  := $*REGALLOC.fresh_o();
    my $lbl_next := MAST::Label.new();
    my $lbl_done := MAST::Label.new();
    push(@ops, MAST::Op.new( :op('elems'), $n_reg, $res_reg ));
    push(@ops, MAST::Op.new( :op('const_i64'), $i_reg, MAST::IVal.new( :value(0) ) ));
    push(@ops, $lbl_next);
    push(@ops, MAST::Op.new( :op('lt_i'), $cmp_reg, $i_reg, $n_reg ));
    push(@ops, MAST::Op.new( :op('unless_i'), $cmp_reg, $lbl_done ));
    push(@ops, MAST::Op.new( :op('atpos_o'), $tmp_reg, $res_reg, $i_reg ));
    push(@ops, MAST::Op.new( :op('hllize'), $tmp_reg, $tmp_reg ));
    push(@ops, MAST::Op.new( :op('bindpos_o'), $res_reg, $i_reg, $tmp_reg ));
    push(@ops, MAST::Op.new( :op('const_i64'), $cmp_reg, MAST::IVal.new( :value(1) ) ));
    push(@ops, MAST::Op.new( :op('add_i'), $i_reg, $i_reg, $cmp_reg ));
    push(@ops, MAST::Op.new( :op('goto'), $lbl_next ));
    push(@ops, $lbl_done);
...
$ops.add_hll_op('perl6', 'p6bindattrinvres', -> $qastcomp, $op {
    ...
    push(@ops, MAST::Op.new( :op('bindattrs_o'), $inv_res.result_reg,
...
my $p6bool := -> $qastcomp, $op {
    ...
    my $exprres;
    my $want := $MVM_reg_obj;
    if istype($op[0], QAST::Var) && $op[0].scope eq 'lexicalref' {
        my $spec := objprimspec($op[0].returns);
    ...
    if $cond_kind == $MVM_reg_int64 {
        ...
        push(@ops, MAST::Op.new( :op('const_n64'), $zero_reg, MAST::NVal.new( :value(0.0) ) ));
        push(@ops, MAST::Op.new( :op('eq_n'), $tmp_reg, $exprres.result_reg, $zero_reg ));
        push(@ops, MAST::ExtOp.new( :op('p6bool'), :cu($qastcomp.mast_compunit),
            $res_reg, $tmp_reg ));
        ...
        my $tmp_reg := $*REGALLOC.fresh_i();
        push(@ops, MAST::Op.new( :op('istrue_s'), $tmp_reg, $exprres.result_reg ));
        push(@ops, MAST::ExtOp.new( :op('p6bool'), :cu($qastcomp.mast_compunit),
            $res_reg, $tmp_reg ));
        $*REGALLOC.release_register($tmp_reg, $MVM_reg_int64);
    }
        ...
        my $tmp_reg := $*REGALLOC.fresh_i();
        push(@ops, MAST::Op.new( :op('decont'), $res_reg, $exprres.result_reg ));
        push(@ops, MAST::Op.new( :op('istrue'), $tmp_reg, $res_reg ));
        push(@ops, MAST::ExtOp.new( :op('p6bool'), :cu($qastcomp.mast_compunit),
            $res_reg, $tmp_reg ));
        $*REGALLOC.release_register($tmp_reg, $MVM_reg_int64);
    }
    else {
        die('Unknown register type in p6bool');
    }
...
$ops.add_hll_op('perl6', 'p6sink', -> $qastcomp, $op {
    ...
    if $sinkee_res.result_kind == $MVM_reg_obj {
        ...
        my $done_lbl   := MAST::Label.new();
        push(@ops, MAST::Op.new( :op('isconcrete'), $itmp, $sinkee_reg ));
        push(@ops, MAST::Op.new( :op('unless_i'), $itmp, $done_lbl ));
        push(@ops, MAST::Op.new( :op('can'), $itmp, $sinkee_reg,
        ...
        push(@ops, MAST::Op.new( :op('findmeth'), $meth, $sinkee_reg,
            MAST::SVal.new( :value('sink') )));
        push(@ops, MAST::Call.new(
...
$ops.add_hll_op('perl6', 'defor', -> $qastcomp, $op {
    if +$op.list != 2 {
        die("Operation 'defor' needs 2 operands");
    }
    my $tmp := $op.unique('defined');
    $qastcomp.as_mast(
         AST {
               $$tmp  :decl := $op[0];
               if $tmp { $$tmp } else { $op[1] }
             }
...
sub boxer($kind, $op) {
    -> $qastcomp, $reg {
        ...
        push(@ops, MAST::ExtOp.new( :op($op), :cu($qastcomp.mast_compunit),
            $res_reg, $reg ));
...
QAST::MASTOperations.add_hll_unbox('perl6', $MVM_reg_int64, -> $qastcomp, $reg {
    ...
    push($il, MAST::Op.new( :op('decont_i'), $res_reg, $reg ));
...
QAST::MASTOperations.add_hll_unbox('perl6', $MVM_reg_num64, -> $qastcomp, $reg {
    ...
    push($il, MAST::Op.new( :op('decont_n'), $res_reg, $reg ));
    $*REGALLOC.release_register($reg, $MVM_reg_obj);
...
QAST::MASTOperations.add_hll_unbox('perl6', $MVM_reg_str, -> $qastcomp, $reg {
    ...
    push($il, MAST::Op.new( :op('decont_s'), $res_reg, $reg ));
    $*REGALLOC.release_register($reg, $MVM_reg_obj);
...
QAST::MASTOperations.add_hll_unbox('perl6', $MVM_reg_uint64, -> $qastcomp, $reg {
    ...
    push($il, MAST::Op.new( :op('decont_u'), $res_reg, $reg ));
    $*REGALLOC.release_register($reg, $MVM_reg_obj);
...
$ops.add_hll_op('perl6', 'p6bindsig', :!inlinable, -> $qastcomp, $op {
    ...
    push(@ops, MAST::Op.new( :op('isnull'), $isnull_result, $bind_res.result_reg ));
    push(@ops, MAST::Op.new( :op('if_i'), $isnull_result, $dont_return_lbl ));
    push(@ops, MAST::Op.new( :op('return_o'), $bind_res.result_reg ));
    push(@ops, $dont_return_lbl);
...
my $trial_bind := -> $qastcomp, $op {
    $qastcomp.as_mast(QAST::Op.new(
        :op('call'),
        QAST::WVal.new( :value(getcodeobj(&trial_bind)) ),
        |@($op)
    ));
};
...
$ops.add_hll_op('nqp', 'p6setbinder', -> $qastcomp, $op {
    $qastcomp.as_mast(QAST::Op.new(
        :op('call'),
        QAST::WVal.new( :value(getcodeobj(&set_binder)) ),
        |@($op)
    ));
});
$ops.add_hll_op('perl6', 'p6typecheckrv', -> $qastcomp, $op {
    if istype($op[1], QAST::WVal) {
        my $type := getcodeobj(&get_binder)().get_return_type($op[1].value);
        if isnull($type) || objprimspec(decont($type)) {
            ...
            push(@ops, MAST::Op.new( :op('decont'), $decont, $value_res.result_reg ));
            push(@ops, MAST::Op.new( :op('istype'), $istype, $decont, $type_res.result_reg ));

            if $emit_definite_check == -1 {
                push(@ops, MAST::Op.new( :op('if_i'), $istype, $lbl_done ));
            } else {
                my $lbl_failed_initial_typecheck    := MAST::Label.new();
                push(@ops, MAST::Op.new( :op('unless_i'), $istype, $lbl_failed_initial_typecheck ));

                push(@ops, MAST::Op.new( :op('isconcrete'), $isdefinite, $decont ));
                if $emit_definite_check == 0 {
                    push(@ops, MAST::Op.new( :op('unless_i'), $isdefinite, $lbl_done ));
                } else {
                    push(@ops, MAST::Op.new( :op('if_i'), $isdefinite, $lbl_done ));
                }
                push(@ops, $lbl_failed_initial_typecheck);
            }

            push_ilist(@ops, $niltype_res);
            push(@ops, MAST::Op.new( :op('istype'), $istype, $decont, $failure_o) );
            push(@ops, MAST::Op.new( :op('if_i'), $istype, $lbl_done ));
            ...
            proto return_error($got, $wanted) {
                my %ex := gethllsym('perl6', 'P6EX');
                if isnull(%ex) || !existskey(%ex, 'X::TypeCheck::Return') {
                    die("Type check failed for return value; expected '" ~
                        $wanted.HOW.name($wanted) ~ "' but got '" ~
                        $got.HOW.name($got) ~ "'");
                }
                else {
                    atkey(%ex, 'X::TypeCheck::Return')($got, $wanted)
                }
            }
            my $err_rep := $qastcomp.as_mast(QAST::WVal.new( :value(getcodeobj(&return_error)) ));
            push_ilist(@ops, $err_rep);
            push(@ops, MAST::Call.new(
                :target($err_rep.result_reg),
                :flags($Arg::obj, $Arg::obj),
        ...
        die('p6dtypecheckrv expects a QAST::WVal as its second child');
    }
});
$ops.add_hll_op('perl6', 'p6decontrv', -> $qastcomp, $op {
    my $is_rw;
    if istype($op[0], QAST::WVal) {
        $is_rw := istrue($op[0].value.rw);
    }
    else {
        die('p6decontrv expects a QAST::WVal as its first child');
    }
    if $is_rw {
        $qastcomp.as_mast($op[1])
    }
    else {
        my $type := getcodeobj(&get_binder)().get_return_type($op[0].value);
        if !isnull($type) && objprimspec(decont($type)) -> int $prim {
            ...
            my $value_res := $qastcomp.as_mast($op[1], :want($MVM_reg_obj));
            push_ilist(@ops, $value_res);
            push(@ops, MAST::ExtOp.new( :op('p6decontrv'), :cu($qastcomp.mast_compunit),
...
op p6configposbindfailover :inlinable
    $qastcomp.as_mast( AST
...
sub push_ilist(@dest, $src) {
    splice(@dest, $src.instructions, +@dest, 0);
}
