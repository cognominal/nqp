...
sub p6ize_recursive($x) {
    if islist($x) {
        my @copy := [];
        for $x {
            push(@copy, p6ize_recursive($_));
        }
        return hllizefor(@copy, 'perl6');
    }
    elsif ishash($x) {
        my %copy := hash();
        for $x {
        ...
        return hllizefor(%copy, 'perl6').item;
    }
    hllizefor($x, 'perl6');
}
...
sub levenshtein($a, $b) {
    ...
    sub changecost($ac, $bc) {
        sub issigil($_) { index('$@%&|', $_) != -1 };
        return 0 if $ac eq $bc;
        return 0.1 if fc($ac) eq fc($bc);
    ...
    sub levenshtein_impl($apos, $bpos, $estimate) {
        ...
        return %memo{$key} if existskey(%memo, $key);
        ...
        my $achar := substr($a, $apos, 1);
        my $bchar := substr($b, $bpos, 1);
        ...
        if $apos + 1 <= $alen && $bpos + 1 <= $blen &&
           eqat($a, $bchar, $apos + 1) && eqat($b, $achar, $bpos + 1) {
...
sub make_levenshtein_evaluator($orig_name, @candidates) {
    ...
    sub inner($name, $hash) {
        ...
        my $parlen := chars($orig_name);
        my $lendiff := chars($name) - $parlen;
        ...
        if $target != -1 {
            my $name-str := box_s($name, $Str-obj);
            push($target, $name-str);
...
class Perl6::World is HLL::World {
    my class Perl6CompilationContext is HLL::World::CompilationContext {
        ...
        method push_lexpad($/) {
            ...
            my $pad := Block Stmts.new :n ) );
            if $*WANTEDOUTERBLOCK {  # (outside of 1st push/pop pass)
                AST $pad :outer(',$*WANTEDOUTERBLOCK);
            }
            elsif +@!BLOCKS {
                $pad.annotate('outer', @!BLOCKS[+@!BLOCKS - 1]);
            }
            AST $pad :statement_id($*STATEMENT_ID) :in_stmt_mod($*IN_STMT_MOD);
            @!BLOCKS[+@!BLOCKS] := $pad;
            $pad
        }
        ...
        method find_lexical_container_type(str $name) {
            ...
            die("Could not find container descriptor for $name");
        }
    ...
    method RAKUDO_MODULE_DEBUG() {
        if isconcrete($!RAKUDO_MODULE_DEBUG) {
            $!RAKUDO_MODULE_DEBUG
        }
    ...
    method loading_and_symbol_setup($/) {
        ...
        my $have_outer := defined(%*COMPILING<%?OPTIONS><outer_ctx>);
        if $have_outer {
            $setting_name := '';
        ...
        if existskey(%*COMPILING<%?OPTIONS>, 'global') {
    ...
    method mop_up_and_check($/) {
        ...
        if $!compiler_services {
            ...
            bindattr($cs, $cs.WHAT, '$!compiler', null());
        }
    }
            ...
            $EXPORTHOW.pairs.map(-> $pair {
                ...
                if $key eq 'SUPERSEDE' {
                    ...
                    for %SUPERSEDE {
                        ...
                        $*LANG.set_how("U:$pdecl", hash('SUPERSEDE', $meta));
                    }
                }
                    ...
                    for %DECLARE {
                        ...
                        $*LANG.set_how($pdecl, $meta);
                        $*LANG.set_how("U:$pdecl", hash('DECLARE', $meta));
                    ...
                    if $key eq lc($key) {
                        ...
                        # before 6.0.0.
                        $*LANG.set_how($key, decont($pair.value));
                    }
                    else {
            ...
            if defined($arglist) {
                ...
                for $arglist -> $tag {
                    if istype($tag, $Pair) {
                        ...
                        push(@positional_imports, $tag);
                    }
                }
            }
            else {
                push(@to_import, 'DEFAULT');
            }
            for @to_import -> $tag {
                if existskey($EXPORT, $tag) {
                    self.import($/, self.stash_hash($EXPORT{$tag}), $package_source_name);
...
#                    $/.check_LANG_oopsies("do_import");
    ...
    method do_pragma($/,$name,$on,$arglist) {
        ...
        if %just_set_pragma{$name} {
            ...
            $*LANG.set_pragma($_.key, $on) if eqat($_.key,'MONKEY',0) for %just_set_pragma;
        }
            ...
            if self.is_precompilation_mode {
                my $automatic_precomp := ifnull(atkey(getenvhash, 'RAKUDO_PRECOMP_WITH'), 0);
                exit(0) if $automatic_precomp;
            ...
            for $arglist -> $arg {
                ...
                elsif istype($arg,$Pair) {
                    my $value := $arg.value;
                    if istype($value,$Bool) && $value {
                        $type := $arg.key;
    ...
    method current_file() {
        my $file := getlexdyn('$?FILES');
        if isnull($file) {
            $file := '<unknown file>';
        }
        elsif !eqat($file,'/',0) && !eqat($file,'-',0) && !eqat($file,':',1) {
            $file := cwd ~ '/' ~ $file;
        }
        $file;
    }
    ...
    method load_module_early($/, $module_name, %opts, $cur_GLOBALish) {
        ...
        my $module := gethllsym('perl6', 'ModuleLoader').load_module($module_name, %opts,
            $cur_GLOBALish, :$line);
        ...
        if self.is_precompilation_mode() {
            ...
            for %opts {
                ...
                if isstr($_.value) || istype($_.value, $Str) {
    ...
    method load_module($/, $module_name, %opts, $cur_GLOBALish) {
        ...
        gethllsym('perl6','ModuleLoader').merge_globals_lexically(self, $cur_GLOBALish, $globalish);
    ...
    method import($/, %stash, $source_package_name) {
        ...
        for %stash {
            if $target.symbol($_.key) -> %sym {
                ...
                if can($installed, 'is_dispatcher') && $installed.is_dispatcher
                && can($foreign, 'is_dispatcher') && $foreign.is_dispatcher {
                    ...
                    if $installed.onlystar && $foreign.onlystar {
                        ...
                        for $foreign.dispatchees {
                            unless existskey(%seen, $_.static_id) {
                                self.add_dispatchee_to_proto($installed, $_);
                            }
                        }
                    }
                    else {
                        push(@clash_onlystar, $_.key);
                    }
                }
                else {
                    push(@clash, $_.key);
    ...
    method install_package($/, @name_orig, $scope, $pkgdecl, $package, $outer, $symbol) {
        ...
        if $create_scope eq 'our' {
            if can($cur_pkg.HOW, 'archetypes') && $cur_pkg.HOW.archetypes.parametric {
    ...
    method install_lexical_symbol($block, str $name, $obj, :$clone) {
        ...
        if isnull(getobjsc($obj)) {
            self.add_object($obj);
        }
        if $block.symbol($name) {
            for @($block[0]) {
                if istype($_, QAST::Var) && $_.name eq $name {
                    $_.decl('static');
                    $_.value($obj);
                    last;
                }
    ...
    method install_lexical_container($block, str $name, %cont_info, $descriptor, :$scope, :$package, :$cont = self.build_container_and_add_to_sc(%cont_info, $descriptor)) {
        ...
        my $prim := %cont_info<sigil> eq '$' && objprimspec($descriptor.of);
        if $prim {
            if $scope eq 'state' { die("Natively typed state variables not yet implemented") }
            if $prim == 1 {
    ...
    method create_container_descriptor($of, $rw, $name, $default = $of, $dynamic = chars($name) > 2 && eqat($name, '*', 1)) {
    ...
    method build_container(%cont_info, $descriptor) {
        ...
        elsif istype($cont_type, self.find_symbol(['Scalar'])) {
            $cont := create($cont_type);
            bindattr($cont, %cont_info<container_base>, '$!descriptor', $descriptor);
            if existskey(%cont_info, 'scalar_value') {
                bindattr($cont, %cont_info<container_base>, '$!value',
                    %cont_info<scalar_value>);
            }
        }
        elsif istype($cont_type, self.find_symbol(['Array'])) ||
              istype($cont_type, self.find_symbol(['Hash'])) {
            $cont := create($cont_type);
            bindattr($cont, %cont_info<container_base>, '$!descriptor', $descriptor);
        }
        else {
            $cont := $cont_type.new;
            try bindattr($cont, %cont_info<container_base>, '$!descriptor', $descriptor);
        }
        $cont
    }
    ...
    method container_type_info($/, $sigil, @value_type, @cont_type, $shape?, :@post) {
        ...
        @value_type[0] := decont(@value_type[0]) if @value_type;
        @cont_type[0] := decont(@cont_type[0]) if @cont_type;
        ...
        if $sigil eq '@' {
            if @cont_type {
                ...
                my $base_type_name     := objprimspec(@value_type[0]) ?? 'array' !! 'Array';
            ...
            if @value_type {
                ...
                %info<container_type>  := self.parameterize_type_with_args($/,
                    %info<container_base>, [$vtype], hash());
                %info<bind_constraint> := self.parameterize_type_with_args($/,
                    %info<bind_constraint>, [$vtype], hash());
                    ...
                    if +@($shape_ast) == 1 {
                        if $shape_ast[0].has_compile_time_value {
                            ...
                            if $op_ast.op eq "call" && +@($op_ast) == 2 {
                                if !isconcrete($op_ast[0].value) && !isconcrete($op_ast[1].value) {
            ...
            if @value_type {
                if objprimspec(@value_type[0]) {
                    self.throw($/, 'X::Comp::NYI',
                      feature => "native value types for hashes");
                }
                %info<container_type>  := self.parameterize_type_with_args($/,
                    %info<container_base>, @value_type, hash());
                %info<bind_constraint> := self.parameterize_type_with_args($/,
                    %info<bind_constraint>, @value_type, hash());
            ...
            if @value_type {
                %info<bind_constraint> := self.parameterize_type_with_args($/,
                    %info<bind_constraint>, [@value_type[0]], hash());
            }
    ...
    method install_lexical_magical($block, $name) {
        ...
        if existskey(%magical_cds, $name) {
            my $mcd := atkey(%magical_cds, $name);
    ...
    method auto_declare_var($var) {
        ...
        die("auto_declare_var") unless objectid($*PACKAGE) == objectid($*LEAF.package);
    ...
    method create_parameter($/, %param_info) {
        ...
        my $primspec := objprimspec(%param_info<nominal_type>);
        if $primspec == 1 {
        ...
        # Populate it.
        if existskey(%param_info, 'variable_name') {
            bindattr_s($parameter, $par_type, '$!variable_name', %param_info<variable_name>);
        }
        bindattr($parameter, $par_type, '$!nominal_type', %param_info<nominal_type>);
        bindattr_i($parameter, $par_type, '$!flags', $flags);
        if %param_info<named_names> {
            bindattr($parameter, $par_type, '@!named_names', %param_info<named_names>);
        }
        if %param_info<type_captures> {
            bindattr($parameter, $par_type, '@!type_captures', %param_info<type_captures>);
        }
        if %param_info<post_constraints> {
            bindattr($parameter, $par_type, '@!post_constraints',
                %param_info<post_constraints>);
        }
        if existskey(%param_info, 'default_value') {
            bindattr($parameter, $par_type, '$!default_value', %param_info<default_value>);
        }
        if existskey(%param_info, 'container_descriptor') {
            bindattr($parameter, $par_type, '$!container_descriptor', %param_info<container_descriptor>);
        }
        if existskey(%param_info, 'attr_package') {
            bindattr($parameter, $par_type, '$!attr_package', %param_info<attr_package>);
        }
        if existskey(%param_info, 'sub_signature') {
            bindattr($parameter, $par_type, '$!sub_signature', %param_info<sub_signature>);
        }
        if existskey(%param_info, 'coerce_type') {
            $parameter.set_coercion(%param_info<coerce_type>);
        }

        if existskey(%param_info, 'dummy') {
    ...
    method create_signature_and_params($/, %signature_info, $lexpad, $default_type_name,
        ...
        if $method {
            my $package := istype($/,NQPMu) ?? $*LEAF.package !! $/;
            ...
            unless has_named_slurpy_or_capture(@params) {
                unless can($package.HOW, 'hidden') && $package.HOW.hidden($package) {
                    @params.push(hash(
                        variable_name => '%_',
        ...
        for @params {
            ...
            unless existskey($_, 'nominal_type') {
            ...
            if existskey($_, 'sub_signature_params') {
            ...
            if $varname {
                ...
                if +%sym && !existskey(%sym, 'descriptor') {
            ...
            if $varname && objprimspec($_<nominal_type>) {
                my $param_type := self.find_symbol(['Parameter'], :setting-only);
                my int $flags := getattr_i($param_obj, $param_type, '$!flags');
                if $flags +& $SIG_ELEM_IS_RW {
                    for @($lexpad[0]) {
                        if istype($_, QAST::Var) && $_.name eq $varname {
                            $_.scope('lexicalref');
                        }
                    }
                }
    ...
    method create_signature(%signature_info) {
        ...
        my $signature  := create($sig_type);
        my @parameters := %signature_info<parameter_objects>;
        self.add_object($signature);

        # Set parameters.
        bindattr($signature, $sig_type, '@!params', @parameters);
        if existskey(%signature_info, 'returns') {
            bindattr($signature, $sig_type, '$!returns', %signature_info<returns>);
        }
        ...
        my int $count := 0;
        my int $i     := 0;
        my int $n     := elems(@parameters);
        while $i < $n {
            my $param := @parameters[$i];
            my int $flags := getattr_i($param, $p_type, '$!flags');
            if $flags +& ($SIG_ELEM_IS_CAPTURE +| $SIG_ELEM_SLURPY_POS +| $SIG_ELEM_SLURPY_LOL +| $SIG_ELEM_SLURPY_ONEARG) {
                $count := -1;
                    ...
                    isnull(getattr($param, $p_type, '@!named_names')) {
                $count++;
                $arity++ unless $flags +& $SIG_ELEM_IS_OPTIONAL;
            }
            $i++;
        }
        bindattr_i($signature, $sig_type, '$!arity', $arity);
        if $count == -1 {
            bindattr($signature, $sig_type, '$!count',
                self.add_constant('Num', 'num', inf()).value);
        } else {
            bindattr($signature, $sig_type, '$!count',
    ...
    method create_simple_code_object($block, $type) {
        ...
        my $sig := self.create_signature(hash('parameter_objects', []));
    ...
    method attach_signature($code, $signature) {
        ...
        my $sig_type := self.find_symbol(['Signature'], :setting-only);
        bindattr($code, $code_type, '$!signature', $signature);
        bindattr($signature, $sig_type, '$!code', $code);
    }
    ...
    method finish_code_object($code, $code_past, $is_dispatcher = 0, :$yada) {
        ...
        my @compstuff;
        bindattr($code, $code_type, '@!compstuff', @compstuff);
        self.context().add_cleanup_task(sub () {
            bindattr($code, $code_type, '@!compstuff', null());
        });
        ...
        my $compiler_thunk := {
            # Fix up GLOBAL.
            bindhllsym('perl6', 'GLOBAL', $*GLOBALish);
            ...
            if $is_dispatcher {
                for getattr($code, $routine_type, '@!dispatchees') {
                    my $cs := getattr($_, $code_type, '@!compstuff');
                    my $past := $cs[0] unless isnull($cs);
                    if $past {
        ...
        my $stub := freshcoderef(sub (*@pos, *%named) {
            unless $precomp {
                $compiler_thunk();
            }
            $precomp(|@pos, |%named);
        ...
        setcodename($stub, $code_past.name);
        bindattr($code, $code_type, '$!do', $stub);
        ...
        if self.is_precompilation_mode() {
            @compstuff[2] := sub ($orig, $clone) {
                my $do := getattr($clone, $code_type, '$!do');
                markcodestub($do);
                self.context().add_cleanup_task(sub () {
                    bindattr($clone, $code_type, '@!compstuff', null());
                });
        ...
        if $is_dispatcher {
            bindattr($code, $routine_type, '@!dispatchees', []);
        }

        # Set yada flag if needed.
        if $yada {
            bindattr_i($code, $routine_type, '$!yada', 1);
        }

        # If it's a routine, store the package to make backtraces nicer.
        if istype($code, $routine_type) {
            die("finish_code_object") unless objectid($*PACKAGE) == objectid($*LEAF.package);
            bindattr($code, $routine_type, '$!package', $*PACKAGE);
        }
    ...
    method run_phasers_code($code, $code_past, $block_type, $type) {
        my @phasers := atkey(getattr($code, $block_type, '$!phasers'), $type);
    ...
    method add_phasers_handling_code($code, $code_past) {
        ...
        if istype($code, $block_type) {
            my %phasers := getattr($code, $block_type, '$!phasers');
            unless isnull(%phasers) {
                if existskey(%phasers, 'PRE') {
    ...
    method setup_let_or_temp($/, $value_stash, $phaser) {
        ...
        self.add_phaser($/, $phaser,
            self.create_code_object($phaser_block, 'Code',
                self.create_signature(hash('parameter_objects', []))));
    }
    ...
    method scalar_wrap($obj) {
        ...
        my $scalar      := create($scalar_type);
        self.add_object($scalar);
        bindattr($scalar, $scalar_type, '$!value', $obj);
        $scalar;
    }
    ...
    method compile_in_context($past, $code_type) {
        ...
        while $cur_block {
            ...
            for %symbols {
                ...
                unless self.is_nested() || %seen{$name} {
                    # Add symbol.
                    my %sym   := $_.value;
                    my $value := existskey(%sym, 'value') || existskey(%sym, 'lazy_value_from')
                        ?? self.force_value(%sym, $name, 0)
                        !! $mu;
                    if isnull(getobjsc($value)) {
        ...
        my int $num_subs := elems(@coderefs);
        my int $i := 0;
        my $result;
        while $i < $num_subs {
            my $subid := getcodecuid(@coderefs[$i]);
            my %sub_id_to_code_object := self.context().sub_id_to_code_object();
            if existskey(%sub_id_to_code_object, $subid) {
                my $code_obj := %sub_id_to_code_object{$subid};
                setcodeobj(@coderefs[$i], $code_obj);
                bindattr($code_obj, $code_type, '$!do', @coderefs[$i]);
                bindattr($code_obj, $code_type, '@!compstuff', null());
            }
            my %sub_id_to_cloned_code_objects := self.context().sub_id_to_cloned_code_objects();
            if existskey(%sub_id_to_cloned_code_objects, $subid) {
                for %sub_id_to_cloned_code_objects{$subid} -> $code_obj {
                    my $clone := clone(@coderefs[$i]);
                    setcodeobj($clone, $code_obj);
                    bindattr($code_obj, $code_type, '$!do', $clone);
                    bindattr($code_obj, $code_type, '@!compstuff', null());
                }
            }
            my %sub_id_to_sc_idx := self.context().sub_id_to_sc_idx();
            if existskey(%sub_id_to_sc_idx, $subid) {
                markcodestatic(@coderefs[$i]);
                self.update_root_code_ref(%sub_id_to_sc_idx{$subid}, @coderefs[$i]);
            }
            if existskey(%!code_object_fixup_list, $subid) {
    ...
    method add_constant($type, $primitive, :$nocache, *@value, *%named) {
        ...
        if !$nocache {
            ...
            if $primitive eq 'bigint' {
                $cache_key := "$type,bigint," ~ tostr_I(@value[0]);
            } else {
                $cache_key := "$type,$primitive,"
                    ~ join(',', @value)
                    ~ $namedkey;
            }
            if existskey(%const_cache, $cache_key) {
        ...
        if $primitive eq 'int' {
            ...
            die("Don't know how to build a $primitive constant");
        }
    ...
    method add_numeric_constant($/, $type, $value) {
        ...
        my $const := self.add_constant($type, lc($type), $value);
        my $past;
        if $type eq 'Int' {
    ...
    method whatever() {
        ...
        unless isconcrete($the_whatever) {
            $the_whatever := create(self.find_symbol(['Whatever']));
    ...
    method hyper_whatever() {
        ...
        unless isconcrete($the_hyper_whatever) {
            $the_hyper_whatever := create(self.find_symbol(['HyperWhatever']));
    ...
    method p6ize_recursive($data, :$dynamic) {
        ...
        if ishash($data) && $dynamic {
            ...
            bindattr($scalar, self.find_symbol(['Scalar']), '$!descriptor', $descriptor);
            $scalar;
        }
        else {
    ...
    method nibble_to_str($/, $ast, $mkerr) {
        if (istype($ast, QAST::Stmts) || istype($ast, QAST::Stmt)) && +@($ast) == 1 {
            $ast := $ast[0];
        }

        $ast.wanted(1);
        if $ast.has_compile_time_value {
            return unbox_s($ast.compile_time_value);
        } elsif istype($ast, QAST::Op) {
            if $ast.name eq '&infix:<,>' {
                my @pieces;
                for @($ast) {
                    if $_.has_compile_time_value {
                        push(@pieces, unbox_s($_.compile_time_value));
                    }
                    else {
                ...
                my $inspect := $ast;
                while istype($inspect, QAST::Op) {
                    $inspect := $inspect[0];
                }
                if istype($inspect, QAST::WVal) && !istype($inspect.value, self.find_symbol(["Block"], :setting-only)) {
                    $/.panic($mkerr());
                }
                else {
                    my $result;
        ...
        } elsif istype($ast, QAST::Var) {
            my $result;
            $result := self.compile_time_evaluate($/, $ast);
            CATCH {
                $/.panic($mkerr());
            }
            return unbox_s($result);
        } else {
            $/.panic($mkerr());
        }
    }
    ...
    method pkg_create_mo($/, $how, :$name, :$repr, :$auth, :$ver, *%extra) {
        ...
        if defined($repr) { %args<repr> := ~$repr; }
        if defined($ver) { %args<ver> := $ver; }
        if defined($auth) { %args<auth> := $auth; }
        if existskey(%extra, 'base_type') {
            %args<base_type> := %extra<base_type>;
        }
        if existskey(%extra, 'group') {
            %args<group> := %extra<group>;
        }
        if existskey(%extra, 'signatured') {
    ...
    method get_attribute_meta_object($/, $name, $later?) {
        my $package := istype($/,NQPMu) ?? $*LEAF.package !! $/;
        unless can($package.HOW, 'get_attribute_for_usage') {
    ...
    method pkg_compose($/, $obj) {
        ...
        if isconcrete($compiler_services) {
    ...
    method get_compiler_services() {
        unless isconcrete($!compiler_services) {
            try {
                my $wtype   := self.find_symbol(['Rakudo', 'Internals', 'CompilerServices']);
                my $wrapped := CompilerServices.new(w => self);
                my $wrapper := create($wtype);
                bindattr($wrapper, $wtype, '$!compiler', $wrapped);
    ...
    method parameterize_type_with_args($/, $role, @pos_args, %named_args) {
        ...
        if @pos_args || %named_args {
            unless can($role.HOW, 'parameterize') {
    ...
    method create_enum_value($enum_type_obj, $key, $value) {
        # Create directly.
        my $val := rebless(clone($value), $enum_type_obj);
        bindattr($val, $enum_type_obj, '$!key', $key);
        bindattr($val, $enum_type_obj, '$!value', $value);
        self.add_object($val);
    ...
    method create_coercion_type($/, $target, $constraint) {
        self.ex-handle($/, {
            ...
            if isnull(getobjsc($type)) { self.add_object($type); }
            $type
        })
    }
    ...
    method suggest_typename($name) {
        ...
        sub evaluator($name, $object, $has_object, $hash) {
            ...
            return 1 if !$has_object || isconcrete($object);
            return 1 if existskey(%seen, $name);
    ...
    method apply_trait($/, $trait_sub_name, *@pos_args, *%named_args) {
        ...
        try {
            ...
            CATCH { $ex := $_; }
            CONTROL {
                if getextype($_) == const::CONTROL_WARN {
                    $/.worry(getmessage($_));
                    resume($_);
                }
                rethrow($_);
        ...
        if $ex {
            my $payload := getpayload($ex);
            if istype($payload, self.find_symbol(["X", "Inheritance", "UnknownParent"], :setting-only)) {
    ...
    method create_lexical_capture_fixup() {
        ...
        my class FixupList {
            ...
            method add_unresolved($code) {
                ...
                if isconcrete($!resolved) {
                    ...
                    unless $added_update {
                        p6captureouters2([$code], $!resolved);
                    }
                }
            }
            method resolve($resolved) {
                ...
                $!resolved := $resolved;
                scwbenable();
                p6captureouters2($!list, $resolved);
            }
            method update($code) {
                if !isnull($!resolved) && !istype($!resolved, NQPMu) {
                    p6captureouters2([$code],
                        getcomp('perl6').backend.name eq 'moar'
                            ?? getstaticcode($!resolved)
                            !! $!resolved);
                }
            }
        }
        ...
        my $fixup_list := create(FixupList);
        self.add_object($fixup_list);
        bindattr($fixup_list, FixupList, '$!list', list());
        bindattr($fixup_list, FixupList, '$!resolver', self.handle());
        ...
        if $phaser eq 'BEGIN' {
            ...
            if $phaser eq 'POST' {
                ...
                if $phaser_past.symbol('$_') {
                    for @($phaser_past[0]) {
                        if istype($_, QAST::Op) && $_.op eq 'bind' && $_[0].name eq '$_' {
                ...
                push(
                    getattr($block.signature, self.find_symbol(['Signature'], :setting-only), '@!params'),
    ...
    my class LongName {
        ...
        method attach_adverbs() {
            if @!colonpairs {
                my $last := pop(@!components) ~ self.canonical_pairs;
        ...
        method type_name_parts($dba, :$decl) {
            ...
            if $decl && $!get_who {
                my $name := self.text;
                die("Name $name ends with '::' and cannot be used as a $dba");
            }
            if +@!components == 1 && self.is_pseudo_package(@!components[0]) {
                ...
                if $c eq 'GLOBAL' {
                    die("Cannot declare pseudo-package GLOBAL");
                }
            ...
            for @!components {
                if istype($_, QAST::Node) {
                    if $_.has_compile_time_value {
                        for split('::', ~$_.compile_time_value) {
                            @name.push($_);
                        }
                    }
                    else {
                        my $name := self.text;
                        die("Name $name is not compile-time known, and can not serve as a $dba");
                    }
                }
        ...
        method fully_qualified_with($target_package) {
            ...
            unless $target_package =:= $*GLOBALish {
                try {
                    my str $tname := getattr_s(
                        decont($target_package.WHO),
                        $*W.find_symbol(['Stash']),
                        '$!longname');
    ...
    method dissect_longname($longname) {
        ...
        my $result := create(LongName);
        bindattr($result, LongName, '$!match', $longname);
        ...
        for $name<morename> {
            if $_<identifier> {
                ...
                if +@components {
                    bindattr_i($result, LongName, '$!get_who', 1);
                }
            }
        }
        bindattr($result, LongName, '@!components', @components);
        ...
        for $longname<colonpair> {
            if $_<coloncircumfix> && !$_<identifier> {
                ...
                if $*COMPILING_CORE_SETTING {
                    ...
                    if istype($ast, QAST::Op) && $ast.name eq '&val' {
                        $ast := $ast[0];
                    }
                    if istype($ast, QAST::Want) && istype($ast[2], QAST::SVal) {
                        $cp_str := self.canonicalize_pair('',$ast[2].value);
                    }
                    elsif istype($ast, QAST::WVal) &&
                          istype($ast.value, $*W.find_symbol(['Str'], :setting-only)) {
        ...
        bindattr($result, LongName, '@!colonpairs', @pairs);
    ...
    method is_type(@name) {
        ...
        try {
            # This throws if it's not a known name.
            $is_name := !isconcrete(self.find_symbol(@name))
        }
        $is_name
    }
    ...
    method already_declared($scope, $curpackage, $curpad, @name) {
        ...
        if $scope eq 'my' && +@name == 1 {
            ...
            elsif existskey($curpackage.WHO, $first_name) {
    ...
    method regex_in_scope($name) {
        ...
        try {
            ...
            $result := istype($maybe_regex, self.find_symbol(['Regex'], :setting-only));
        }
        $result
    }
    ...
    method walk_symbols($code) {
        ...
        sub walk_block($block) {
            ...
            for %symtable -> $symp {
                my $key := $symp.key;
                my %sym := $symp.value;
                my $has_val := existskey(%sym, 'value') || existskey(%sym, 'lazy_value_from');
    ...
    method find_symbol(@name, :$setting-only, :$upgrade_to_global, :$cur-package) {
        ...
        unless +@name { die("Cannot look up empty name"); }
        ...
        if +@name == 1 {
            ...
            die("find_symbol1") unless objectid($*PACKAGE) == objectid($*LEAF.package);
            $cur-package := $*LEAF.package unless $cur-package;
            if existskey($cur-package.WHO, $final_name) {
                return atkey($cur-package.WHO, $final_name);
            }
        }
        ...
        if +@name >= 2 {
            ...
            unless $found {
                die("find_symbol2") unless objectid($*PACKAGE) == objectid($*LEAF.package);
                $cur-package := $*LEAF.package unless $cur-package;
                if existskey($cur-package.WHO, $first) {
                    $result := atkey($cur-package.WHO, $first);
                    @name := clone(@name);
                    @name.shift();
                }
            }
        ...
        for @name {
            if existskey($result.WHO, ~$_) {
                $result := ($result.WHO){$_};
            }
            else {
                die("Could not locate compile-time value for symbol " ~
                    join('::', @name));
            }
        }
    ...
    method suggest_lexicals($name) {
        ...
        sub evaluate($name, $value, $has_value, $hash) {
            ...
            return 1 unless existskey($hash, "scope");
    ...
    method suggest_routines($name) {
        ...
        if !$with_sigil {
            ...
            for @suggestions {
                push( @no_sigils, substr($_,1,chars($_) - 1) );
            }
    ...
    method is_lexically_visible($name, $scope) {
        ...
        while $cur_block {
            my %symbols := $cur_block.symtable();
            if existskey(%symbols, $name) {
                my %sym := %symbols{$name};
                self.force_value(%sym, $name, 0);
                return existskey(%sym, 'value') &&
                    !isconcrete(%sym<value>) ?? 2 !! 1;
            }
            $cur_block := $cur_block.ann('outer');
        }
    }
    ...
    method force_value(%sym, $key, int $die) {
        if existskey(%sym, 'value') {
            %sym<value>
        }
        elsif existskey(%sym, 'lazy_value_from') {
            %sym<value> := atkey(atkey(%sym, 'lazy_value_from'), $key)
        }
        else {
            $die ?? die("No compile-time value for $key") !! NQPMu
        }
    }
    ...
    method group_exception(*@panic) {
        ...
        %opts<filename> := box_s(self.current_file,self.find_symbol(['Str']));
        try {
            my $group_type := self.find_symbol(['X', 'Comp', 'Group'], :setting-only);
            return $group_type.new(|%opts);
            CATCH {
                print("Error while constructing error object:");
                say($_);
            }
        }
    }
    ...
    method typed_exception($/, $ex_type, *%opts) {
        ...
        unless $*COMPILING_CORE_SETTING {
            try {
                ...
                $type_found := 1;
                $ex := self.find_symbol(
                    islist($ex_type) ?? $ex_type !! split('::', $ex_type),
                    :setting-only);
                my $x_comp := self.find_symbol(['X', 'Comp'], :setting-only);
                unless istype($ex, $x_comp) {
        ...
        if $type_found {
            ...
            if $ex.HOW.name($ex) eq 'X::Syntax::Confused' {
                ...
                if HLL::Compiler.lineof($c.orig, $qe, :cache(1)) >= HLL::Compiler.lineof($c.orig, $c.pos, :cache(1)) - 1
                    && index(substr($c.orig, $qs, $qe - $qs), "\n") >= 0 {
                    my $quotes :=
                        substr($c.orig, $qs - 1 , 1) ~
                        substr($c.orig, $qe, 1);
                    $quotes := "<<>>" if $quotes eq '<>' && eqat($c.orig, '>', $qe + 1);
            ...
            %opts<pos>             := $c.pos unless existskey(%opts, 'pos');
            ...
            for %opts -> $p {
                if islist($p.value) {
                    my @a := [];
                    for $p.value {
                        push(@a, hllizefor($_, 'perl6'));
                    }
                    %opts{$p.key} := hllizefor(@a, 'perl6');
                }
                else {
                    %opts{$p.key} := hllizefor($p.value, 'perl6');
                }
            }
            %opts<filename> := box_s(self.current_file,self.find_symbol(['Str']));
        ...
        sub safely_stringify($target) {
            if $has_str && istype($target, $Str) {
                return ~unbox_s($target);
            } elsif $has_int && istype($target, $Int) {
                return ~unbox_i($target);
            } elsif $has_list && istype($target, $List) {
                my $storage := getattr($target, $List, '$!reified');
                my @result;
                for $storage {
                    push(@result, safely_stringify($_));
                }
                return "(" ~ join(", ", @result) ~ ")";
            } elsif ishash($target) {
                my @result;
        ...
        my @err := ['Error while compiling, type ', (islist($ex_type) ?? join('::', $ex_type) !! $ex_type),  "\n"];
        for %opts -> $key {
            @err.push: '  ';
            @err.push: ~$key;
            @err.push: ': ';
        ...
        findmethod(HLL::Grammar, 'panic')($/, join('', @err));
    }

    method locprepost($c) {
        my $orig := $c.orig;
        my $marked := $c.MARKED('ws');
        my $pos  := $marked && index(" }])>»", substr($orig, $c.pos, 1)) < 0 ?? $marked.from !! $c.pos;
    ...
    method add_additional_frames($frames) {
        if %*COMPILING<%?OPTIONS><mast_frames> {
            ...
            my $iterator := iterator($frames);
            while $iterator {
                my $pair := shift($iterator);
                %existing{iterkey_s($pair)} := iterval($pair);
            }
        }
        else {
    ...
    method handle-begin-time-exceptions($/, $use-case, $code) {
        ...
        if $found_xcbt {
            ...
            $xcbt.SET_FILE_LINE(
                box_s(self.current_file,self.find_symbol(['Str'])),
                box_i(self.current_line($/),self.find_symbol(['Int'])),
            );
            $xcbt.throw;
        }
        else {
    ...
    method rethrow($/, $err) {
        ...
        my $coercer;
        try { $coercer := self.find_symbol(['&COMP_EXCEPTION']); ++$success; };
        rethrow($err) unless $success;
        my $p6ex := $coercer($err);
        unless can($p6ex, 'SET_FILE_LINE') {
            try {
                my $x_comp := self.find_symbol(['X', 'Comp'], :setting-only);
                $p6ex.HOW.mixin($p6ex, $x_comp).BUILD_LEAST_DERIVED(hash());
            }
        }
        if can($p6ex, 'SET_FILE_LINE') {
            $p6ex.SET_FILE_LINE(
                box_s(self.current_file,self.find_symbol(['Str'])),
                box_i(self.current_line($/),self.find_symbol(['Int'])),
            );
